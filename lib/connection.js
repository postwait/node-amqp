...
function objectProxy(target, source, overwrite) {
  // if ('object' !== typeof source) { return;}
  // Get values from prototype
  var bproto = Object.getPrototypeOf(source);
  var s, g, i, btype;
  for (i in bproto) {
    
    if (!overwrite && 'undefined' !== typeof target[i]) {
      continue;
    }
    btype = typeof source[i];
    g = bproto.__lookupGetter__(i), s = bproto.__lookupSetter__(i);

    if (g || s) {
      if (g) {
        target.__defineGetter__(i, proxyGetter.bind(source, source, i));
      }
      if (s) {
        target.__defineSetter__(i, proxySetter.bind(source, source, i));
      }
    } else if ('function' === btype) {
      target[i] = proxyMethod.bind(source, source, i);
    }
  }
  ...
  for (i in source) {
    ...
    g = source.__lookupGetter__(i), s = source.__lookupSetter__(i);
  ...
  if(Array.isArray(this.options.host) === true){
    if(this.hosti == null){
      if(typeof this.options.hostPreference == 'number') {
        ...
        this.hosti = Math.random() * this.options.host.length;
      }
    } else {
