...
Connection.prototype._createSocket = function() {
  ...
  var methods = ['end', 'destroy', 'pause', 'resume', 'setEncoding', 'ref', 'unref', 'address'];
  ...
  self.write = function () {
      var flushed = self.socket.write.apply(self.socket, arguments);
      if (!flushed) {
          /* Buffer must not be reused until all data has been flushed
           * so the easiest way is just to allocate a new buffer.
           */
          self._sendBuffer = new Buffer(maxFrameBuffer);
      }
      return flushed;
  };
};
